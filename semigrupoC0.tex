\chapter{Semigrupos de Operadores Lineares}
...
Dizemos que um semigrupo $S$ é \dt{de classe $C^0$ ou fortemente contínuo} se
\begin{equation}
\lim\limits_{t\to0^+} \|(S(t)-\id)x\|_X =0,\ \forall x\,\in X.
\end{equation}

Dizemos que um semigrupo $S$ é \dt{uniformemente contínuo} se 
\begin{equation}
...
\begin{proof}
\textbf{\underline{Afirmação 1}:} Existem $\delta >0$ e $M\geq 1$ tais 
\begin{equation*}
\|S(t) \|_{\mathcal L (X)} \leq M \text{ para todo } t\in [0,\delta].
\end{equation*}

De fato, se a Afirmação 1 não se verificasse, para cada inteiro $n\geq 1$, existitia $t_n \in \bigg( 0,\frac{1}{n} \bigg)$ tal que
\[
...
\textbf{\underline{Afirmação 2}:} $M\geq 1$.
...
\textbf{\underline{Afirmação 3}:} Existe $\mu \geq 0$ (dependendo das constantes $M\geq $)

à família  $\{S(t)\}_{t\geq 0}$ em $\mathcal{L}(X)$, onde $X$ é Banach. Para isso, basta mostrar que esta família é pontualmente limitada. 
...
\|S(t)\|_{\mathcal{L}(X)}\leq M,\, \text{ para todo  } t\in [0,\delta]. 
\end{equation}
Além disso, $M\geq \|S(0)\|_{\mathcal{L}(X)}=\|\id\|_{\mathcal{L}(X)}=1$.
...
Do item (3) da Definição  \ref{def-semigrupo} , já temos
  \[\|S(h)x-S(0)x\|=\|(S(h)-\id)x\|\to 0,\, \text{ quando } h\to 0.\]
Isto é, $S(\cdot)x$ é contínua em $t=0$. Dado $t\in (0,+\infty)$, se $h>0$, então
\begin{align*}
    \|S(t+h)x-S(t)x\|& =\|S(t)S(h)x-S(t)x\|=\|S(t)(S(h)-\id)x\|\\
    & \leq \|S(t)\|_{\mathcal{L}(X)}\|(S(h)-\id)x\|\\
    & \leq Me^{\mu t}\|(S(h)-\id)x\|\to 0,\, \text{ quando } h\to 0^+.
\end{align*}

Se {\color{pink} $-t<h<0$, então seja $k=-h>0$}. Daí, se $h\to 0^+$, então $k\to0^+$. Com isso,
\begin{align*}
    \|S({\color{pink} t+h})x-S(t)x\|& = \|S(t-k)x-S(t)x\|=\|S(t-k)x-S(t-k+k)x\|\\
    & =\|S(t-k)x-S(t-k)S(k)x\|=\|S(t-k)(\id-S(k)x\|\\
    & \leq \|S(t-k)\|_{\mathcal{L}(X)}\|(S(k)-\id)x\|\\
    & \leq Me^{\mu t}\|(S(k)-\id)x\|\to 0,\, \text{ quando } k\to 0^+.
\end{align*}
\end{proof}
...
\begin{theorem}[Existência e Unicidade do PVI]\label{PVI}
...
%%--------------- regularidade  -------------------------------
% \begin{definition}
%     Seja $S$ um semigrupo de classe $C^0$ em $X$ e $A$ seu gerador infinitesimal. Defina $A^0=\id$, $A^1=A$ e, supondo que $A^{n-1}$ esteja definido, vamos definir $A^n$ pondo:
...
% \begin{tcolorbox}
% \noindent\textbf{Hipótese de Indução:} Suponha que, para algum $m>1$, se $x \in {D}\left(A^m\right)$, então vale a fórmula de Taylor:
% \[
% S(t) x=\sum_{k=0}^{m-1} \frac{(t-a)^k}{k!} A^k S(a) x+\frac{1}{(m-1)!} \int_a^t(t-\tau)^{m-1} A^m S(\tau) x d \tau
% \]
% \end{tcolorbox}
% Vamos mostrar que também vale para $n=m+1$. Par isso, vamos fazer uma integação por partes e usar a fórmula do item $(ii)$. 
% \begin{align*}
% &\frac{1}{m!}\int_a^t (t-\tau)^mA^{m+1}S(t)x\,d\tau\\
% & \quad \stackrel{(ii)}{=}\frac{1}{m!}\int_a^t (t-\tau)^m\frac{d^{m+1}}{d\tau^{m+1}}S(t)x\,d\tau\\
% &\quad =\frac{1}{m!}(t-\tau)^m\frac{d^m}{dt^m}S(t)x\Big\vert_{\tau=a}^{\tau=t}-\frac{1}{m!}\int_a^t -m(t-\tau)^{m-1}\frac{d^m}{dt^m}S(t)x\, d\tau\\
% &\quad \stackrel{(ii)}{=}\frac{1}{m!}(t-\tau)^mA^mS(t)x\Big\vert_{\tau=a}^{\tau=t}
% +\frac{1}{(m-1)!}\int_a^t (t-\tau)^{m-1} A^mS(t)x\\
% &\quad =-\frac{1}{m!}(t-a)^mA^mS(a)x+\frac{1}{(m-1)!}\int_a^t (t-\tau)^{m-1} A^mS(t)x\\
% \shortintertext{(Hipótese de Indução)}
% &\quad= -\frac{1}{m!}(t-a)^mA^mS(a)x+S(t)x-\sum_{k=0}^{m-1} \frac{(t-a)^k}{k!} A^k S(a) x\\
% &\quad= S(t)x-\sum_{k=0}^{m} \frac{(t-a)^k}{k!} A^k S(a) x.
% \end{align*}
% Logo, 
% \[
% S(t)x=\sum_{k=0}^{m} \frac{(t-a)^k}{k!} A^k S(a) x+\frac{1}{m!}\int_a^t (t-\tau)^mA^{m+1}S(t)x\,d\tau,
% \]
% como queríamos.

% \item {\color{red} COMPLETAR A PROVA}
